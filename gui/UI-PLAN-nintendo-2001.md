...
│ 单文件 | 批量  ·  设置  ·  日志目录                   │
├─ periwinkle 机身 ─────────────────────────────────────┤
│ 中部随模式切换（QStackedWidget，非路由栈）：            │
│  · 单文件：drop plate + 任务 form                     │
│  · 批量：队列表 + 批量参数（多文件/文件夹）            │
│ 右侧 rail：设备 / 编码 / 分句摘要 + [设置…]            │
│ ┌ 进度 plate ───────────────────────────────────────┐ │
│ │ 阶段标签 · bevel 进度条 · 状态文案                  │ │
│ │ [生成字幕 / 开始批量 SIGNAL]  [取消 carbon]         │ │
│ └────────────────────────────────────────────────────┘ │
│ ┌ 结果 plate（单文件完成后展开）────────────────────┐ │
...
3. `window.py` — 顶栏 + 次条（单文件|批量）+ 双栏 + footer；DropZone 斜切；`batch_panel.py` 批量页。
4. 字体：捆绑 **Noto Sans SC**（`gui/fonts/NotoSansSC/`）——Regular / Medium / Bold / Black。
5. 禁止：软阴影、大圆角卡片、装饰渐变；圆角仅 pill 等小控件。
...
- [x] 一眼是「机箱面板」，不是扁平白页
- [x] 主前进色为 Signal Orange（生成 / 开始批量）
- [x] Amber 用于工具动作（选择 / 浏览等）
- [x] 板件有上亮下暗 bevel，无 blur shadow
- [x] 中文与分句说明仍可读

## 7. 非目标

不用网页壳；不复制任天堂商标与角色。CLI/管道能力仍只经 subprocess；批量队列走 `qsub batch`（见 `gui/README.md`）。
